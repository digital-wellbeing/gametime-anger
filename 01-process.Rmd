...
if (!dir.exists(here("data")) || length(list.files(here("data"))) == 0 ){
  ...
  download.file("https://osf.io/c2e93/download", here("data", "telemetry-apex-legends.csv.gz"), mode = "wb")
  download.file("https://osf.io/xam5t/download", here("data", "telemetry-outriders.csv.gz"), mode = "wb")
}
```
...
d$Responded <- apply(
  select(d, Angry:hours_est, Age:Experience), 1,
...
```{r}
write_csv(d, file = here("data", "cleaned-data.csv.gz"))
```